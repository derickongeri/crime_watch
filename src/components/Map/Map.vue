...
export default {
  ...
  computed: {
    ...
    getYear() {
      ...
      if (startYear && endYear) return ` ${startYear} to ${endYear}`;
  ...
  watch: {
    ...
    getGeometryData: {
      handler(val) {
        if (val) return this.showSelectedRegionGeometry(val)
      }
    },
    // watch for analysis results data change
    getAnalysisResults: {
      handler(val) {
        if (val) return this.processLayerResult(val)
      }
    },
    getChartData: {
      deep: true,
  ...
  methods: {
    ...
    showSelectedRegionGeometry(val) {
    ...
    async downloadTiffFile() {
      try {
        ...
        let response = await this.$api.get(this.tiff_url, {
